window.MCCDocContent = {
  ...
  "docs/TASK_CREATION_GUIDE.md": [
    ...
    "",
    "**Good:** *In `task-app.js`, add a `dueDate` field to `normalizeTask()` with a default of `null`. Add a date input to the New Task modal after the notes field.*",
    "",
    "**Too vague:** *Make the task modal better.*",
    "",
    "### Section 2 — Why it matters",
    "",
    "Why does this task exist? What breaks, stalls, or suffers if it doesn't get done?",
    "",
    "**Good:** *Without a pricing page, users who land on LBM have no way to understand what paid tiers exist. The funnel has no conversion point.*",
    "",
    "### Section 3 — Why it's valuable",
    "",
    "What does completing this unlock? Name the revenue tier it serves:",
    "",
    "- **$1 tier** — enables the first transaction / first paying user",
    "- **$10 tier** — enough utility to justify recommending to someone else",
    "- **$100 tier** — retention / polish that users pay monthly",
    "- **$1,000 tier** — higher price point or team plan feature",
    "- **$1M tier** — scalable infrastructure, virality, or enterprise readiness",
    "",
    "### Section 4 — Why it's urgent",
    "",
    "What makes this time-sensitive now versus later? Is there a dependency or compounding cost if deferred?",
    "",
    "---",
    "",
    ...
    "",
    "A single task can have multiple tags. A task that redesigns the upgrade button to improve conversion is tagged `[\"monetization\", \"ux\"]`.",
    "",
    "---",
    "",
    "## Urgency Scale",
    ...
    "- \"Create a task for X\"",
    "- \"Log this as a task\" / \"Track this in LBM\"",
    "- \"Make a task: X\" / \"LBM task: X\"",
    "- \"Put this on the board\"",
    "",
    "Claude reads `SKILL_ADD_TASK.md`, infers urgency/effort/value/area/tags from context, builds the 4-section description, sets `addedBy` to its own model ID, shows a draft, and writes it directly to `data/tasks.json`. Reload the browser tab and the task appears.",
    "",
    ...
    "  addedBy:     \"claude-sonnet-4-6\",   // set when Claude creates; leave \"\" for manual",
    ...
    "",
    "Open `data/tasks.json`, add a task object, then run `node scripts/sync-tasks.js`. See [File Sync](docs.html?doc=docs/FILE_SYNC.md).",
    "",
    "### Method 4 — Inline form in the app",
    "",
    "Press `N` or click **+ Add task** in List View. Edit description and tags in the Detail Panel after saving.",
    "",
    "---",
    "",
    ...
    "| Mistake | Instead |",
    "|---|---|",
    "| Title is a vague noun phrase | Use an action verb — \"Add welcome modal\" not \"Better onboarding\" |",
    ...
    "| Urgency 5 on everything | Reserve 5 for genuine blockers |",
    "| Tags left empty | Assign at least one tag |",
    "| \"What to do\" is one sentence | Write it as an AI prompt — specific enough to start immediately |",
    "| Effort left at default for obvious big/small tasks | Set effort accurately — it signals how to schedule the task |",
  ...
  "docs/AI_TASK_CREATION.md": [
    "# AI Task Creation",
    "",
    "LBM supports several AI-powered ways to create tasks: **Claude Code skill** (file-based, recommended), **browser console**, and **voice input**.",
    "",
    "For the full guide on writing high-quality tasks — including the 4-section description format and revenue-tier tagging — see [Task Creation Guide](docs.html?doc=docs/TASK_CREATION_GUIDE.md).",
    "",
    "---",
    "",
